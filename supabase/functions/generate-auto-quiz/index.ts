 ...
 serve(async (req) => {
   ...
   try {
     ...
     // Filter out themes that were already used recently
     const availableThemes = QUIZ_THEMES.filter(
       (t) => !recentKeys.has(`${t.book}::Quiz: ${t.title}`)
     );
 
     // If all themes used, reset to full list
 ...
 Para cada pergunta:
 - Faça uma pergunta clara e específica sobre o tema
 - Ofereça 4 opções de resposta plausíveis
 - Indique qual é a resposta correta (0-3)
 - Adicione uma explicação com referência bíblica
   ...
   "questions": [
     {
       ...
       "correct_answer": 0,
       "explanation": "Explicação com referência bíblica (ex: Gênesis 1:1)"
     }
   ]
 }`;
