...
const Perfil = () => {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setEditForm({
        ...
        birth_date: profile.birth_date || "", bio: (profile as any).bio || "",
      });
    }
  }, [profile]);
  ...
  const fetchStats = async () => {
    const [attendanceRes, progressRes, achievementsRes, viewsRes, postsRes] = await Promise.all([
    ...
    setStats({
      presencas: attendanceRes.count || 0, estudos: progressRes.count || 0,
      conquistas: achievementsRes.count || 0, views: viewsRes.count || 0,
  ...
  const fetchLevelInfo = async () => {
    ...
    const [{ data: levelData }, { data: nextLevelData }] = await Promise.all([
    ...
    if (levelData?.[0]) {
      setLevelInfo({ title: levelData[0].title, icon: levelData[0].icon, xpRequired: levelData[0].xp_required, nextXp: nextLevelData?.xp_required || levelData[0].xp_required + 100 });
    }
  };

  const fetchMediaGallery = async () => {
    const { data } = await supabase.from("community_posts").select("id, image_url, created_at")
      .eq("user_id", user?.id).not("image_url", "is", null).order("created_at", { ascending: false }).limit(12);
    setMediaGallery((data || []).filter(p => p.image_url) as any);
  };

  const handleLogout = async () => {
    await signOut();
    toast({ title: "Até logo! 🙏", description: "Volte sempre. Deus te abençoe!" });
    navigate("/auth");
  };

  const handleUpdateProfile = async () => {
    const { error } = await supabase.from("profiles")
      .update({ full_name: editForm.full_name, phone: editForm.phone || null, birth_date: editForm.birth_date || null, bio: editForm.bio || null })
      .eq("user_id", user?.id);
    if (error) toast({ title: "Erro", description: "Não foi possível atualizar o perfil.", variant: "destructive" });
    else { toast({ title: "Perfil atualizado! ✅" }); setIsEditDialogOpen(false); }
  };
  ...
  const xpProgress = levelInfo ? Math.min(((totalXp - levelInfo.xpRequired) / (levelInfo.nextXp - levelInfo.xpRequired)) * 100, 100) : 0;
  ...
  return (
    <div className="relative min-h-screen bg-background overflow-hidden"
      ...
      <main className="relative z-10 px-4 -mt-16 max-w-2xl mx-auto"
        style={{ paddingLeft: 'max(1rem, env(safe-area-inset-left, 16px))', paddingRight: 'max(1rem, env(safe-area-inset-right, 16px))' }}>
        {/* Avatar and Name */}
        <div className="flex flex-col items-center sm:flex-row sm:items-end sm:gap-4">
          <div className="relative">
            <AvatarUpload userId={user?.id || ""} currentAvatarUrl={avatarUrl} userName={userName} onAvatarChange={setAvatarUrl} />
            {levelInfo && (
          ...
          <div className="flex-1 text-center sm:text-left mt-4 sm:mt-0 sm:mb-2">
            <div className="flex items-center justify-center sm:justify-start gap-2">
              ...
              <button onClick={() => setIsEditDialogOpen(true)} className="text-muted-foreground hover:text-primary transition-colors">
            ...
            <div className="flex items-center justify-center sm:justify-start gap-1 text-xs text-muted-foreground mt-1">
              <Mail className="h-3 w-3" /><span className="truncate max-w-[200px]">{userEmail}</span>
            </div>
            <Badge className={`${getRoleBadgeClass()} px-3 py-1 mt-2`}><Shield className="mr-1 h-3 w-3" />{getRoleLabel()}</Badge>
          </div>
          <Button variant="outline" className="mt-4 sm:mt-0 sm:mb-2 rounded-xl" onClick={() => setIsEditDialogOpen(true)}>
            <Edit className="h-4 w-4 mr-1" /> Editar Perfil
          </Button>
        </div>

        {(profile as any)?.bio && (
          <p className="mt-4 text-muted-foreground text-sm text-center sm:text-left">{(profile as any).bio}</p>
        )}

        {/* XP Progress */}
        {levelInfo && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mt-4 p-4 rounded-2xl bg-card shadow-md">
            ...
            <p className="text-xs text-muted-foreground mt-2 text-center">{Math.round(levelInfo.nextXp - totalXp)} XP para o próximo nível</p>
          </motion.div>
        )}

        {/* Stats Grid - Enhanced */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="mt-4 grid grid-cols-3 sm:grid-cols-5 gap-2">
          {[
            ...
            { label: "Posts", value: stats.posts, icon: MessageSquare, action: () => navigate("/comunidade") },
          ].map((stat) => (
            <motion.button key={stat.label} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              onClick={stat.action} disabled={!stat.action}
              className="rounded-xl bg-card p-3 text-center shadow-md disabled:cursor-default">
        ...
        {mediaGallery.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-semibold text-sm text-foreground">Galeria de Mídia</h3>
              <span className="text-xs text-muted-foreground">({mediaGallery.length})</span>
            </div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="mt-8 pb-4 text-center">
          <p className="font-serif italic text-muted-foreground">"Tudo quanto fizerdes, fazei-o para a glória de Deus."</p>
          <p className="mt-1 text-sm font-medium text-primary">— 1 Coríntios 10:31</p>
        </motion.div>
      </main>

      <div className="absolute z-40" style={{ top: "6rem", right: "max(1rem, env(safe-area-inset-right, 16px))" }}>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="rounded-2xl max-h-[85vh] overflow-hidden flex flex-col">
          <DialogHeader><DialogTitle className="font-serif">Configurações do Perfil</DialogTitle></DialogHeader>
          <Tabs defaultValue="profile" className="flex-1 overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto">
              <TabsContent value="profile" className="mt-0 space-y-4">
                <div><Label>Nome completo</Label><Input value={editForm.full_name} onChange={(e) => setEditForm({ ...editForm, full_name: e.target.value })} className="rounded-xl" /></div>
                <div><Label>Bio (sobre você)</Label>
                  <Textarea value={editForm.bio} onChange={(e) => setEditForm({ ...editForm, bio: e.target.value })} placeholder="Conte um pouco sobre você..." className="rounded-xl resize-none" maxLength={160} rows={3} />
                  <p className="text-xs text-muted-foreground text-right mt-1">{editForm.bio.length}/160</p>
                </div>
                <div><Label>Telefone</Label><Input value={editForm.phone} onChange={(e) => setEditForm({ ...editForm, phone: e.target.value })} placeholder="(00) 00000-0000" className="rounded-xl" /></div>
                <div><Label>Data de nascimento</Label><Input type="date" value={editForm.birth_date} onChange={(e) => setEditForm({ ...editForm, birth_date: e.target.value })} className="rounded-xl" /></div>
                <Button onClick={handleUpdateProfile} className="w-full rounded-xl">Salvar Alterações</Button>
              </TabsContent>
              <TabsContent value="location" className="mt-0">
                <LocationSettings userId={user?.id || ""} currentState={(profile as any)?.state || null} currentCity={(profile as any)?.city || null} onUpdate={() => {}} />
              </TabsContent>
