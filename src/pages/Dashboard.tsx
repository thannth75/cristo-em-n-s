...
const Dashboard = () => {
  ...
  return (
    ...
    >
      <GlowOrb className="absolute -top-20 -right-20 h-48 w-48 opacity-20" />
      <AppHeader userName={userName} />

      <main
        className="relative z-10 px-4 py-4 max-w-4xl mx-auto"
        style={{
        ...
        >
          ...
          {gamification.currentLevelDef && (
            <motion.button
              ...
              className="w-full rounded-2xl bg-card border border-border/50 p-3.5 shadow-sm text-left"
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2.5">
                  <span className="text-xl">{gamification.currentLevelDef.icon}</span>
                  <div>
                    <p className="text-[10px] text-muted-foreground">Nível {gamification.currentLevel}</p>
                    <h3 className="font-semibold text-sm text-foreground">{gamification.currentLevelDef.title}</h3>
                  </div>
                </div>
                <div className="flex items-center gap-1 rounded-full bg-primary/10 px-2.5 py-1">
                  <Sparkles className="h-3 w-3 text-primary" />
                  <span className="font-bold text-sm text-primary">{gamification.totalXp}</span>
          ...
          {nextEvent && (
            <motion.button
              ...
              className="w-full overflow-hidden rounded-2xl gradient-hope p-4 text-primary-foreground shadow-md text-left"
            >
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium opacity-80">Próximo Evento</p>
                  <h3 className="font-serif text-lg font-semibold truncate">{nextEvent.title}</h3>
                  <p className="mt-0.5 text-xs opacity-70">
                    {formatEventDate(nextEvent.event_date).weekday}, {nextEvent.start_time.slice(0, 5)}
                  </p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <div className="flex h-14 w-14 flex-col items-center justify-center rounded-xl bg-primary-foreground/15">
                    <span className="text-xl font-bold">{formatEventDate(nextEvent.event_date).day}</span>
                    <span className="text-[10px] font-medium">{formatEventDate(nextEvent.event_date).month}</span>
                  </div>
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-serif text-sm font-semibold text-foreground">Obra em Restauração</h3>
                <p className="text-[10px] text-muted-foreground mt-0.5 mb-2">
                  Site oficial e rádio do ministério
                </p>
          ...
          <div>
            ...
            <div className="hidden sm:grid grid-cols-2 gap-2 lg:grid-cols-3">
              {features.map((feature, index) => (
                <FeatureCard key={feature.title} {...feature} delay={0.03 * index} />
              ))}
            </div>
          </div>
          ...
          <motion.div
            ...
            className="rounded-2xl bg-accent/30 border border-border/30 p-4 text-center"
          >
