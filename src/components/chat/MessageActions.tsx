...
export function MessageActions({
  ...
  const handleEdit = async () => {
    ...
    if (error) {
      ...
      onMessageEdited(messageId, editContent);
    }
    setIsEditing(false);
    setShowMenu(false);
  };
  ...
  const handleReaction = async (reaction: string) => {
    ...
    if (existing) {
      await supabase.from("message_reactions").delete().eq("message_id", messageId).eq("user_id", userId).eq("reaction", reaction);
  ...
  return (
    <div className="relative">
        ...
        {showReactions && (
          ...
          >
            {QUICK_REACTIONS.map((r) => {
              const hasReacted = reactions.some((rx) => rx.reaction === r && rx.user_id === userId);
              return (
                <button
                  key={r}
                  onClick={() => handleReaction(r)}
                  className={`text-lg hover:scale-125 transition-transform ${hasReacted ? "opacity-100" : "opacity-70"}`}
                >
                  {r}
                </button>
              );
      ...
      {/* Action buttons - show on long press area */}
      ...
      <AnimatePresence>
        {showMenu && (
          ...
          >
            <button
              onClick={() => { setIsEditing(true); setEditContent(content); setShowMenu(false); }}
...
export function MessageReactionsDisplay({ reactions, isOwn }: MessageReactionsDisplayProps) {
  ...
  return (
    <div className={`flex gap-1 mt-0.5 ${isOwn ? "justify-end" : "justify-start"}`}>
      {Object.entries(grouped).map(([emoji, count]) => (
        <span
          key={emoji}
          className="inline-flex items-center gap-0.5 rounded-full bg-muted/80 px-1.5 py-0.5 text-xs"
        >
